...
from workers.llm.storyboard import (
    generate_pictures, generate_pictures_streaming_policy,
    generate_keyframe_prompts_batched,
    persist_named_json, persist_named_text, new_run_id,
    _parse_json_list_strict, _extract_top_level_json, _model_repair_to_json_array
)
...
def storyboardn_round1(req: Round1Req):
    ...
    if not isinstance(pics_json, list) or len(pics_json) == 0:
        raise HTTPException(500, detail={"error": "round1_empty", "failures": meta.get("failures", [])})
    stem = new_run_id()
    ...
    return {
        ...
        "shots": len(pics_json),
        "downloads": {"pictures_url": pics_url, "round1_raw_url": raw_url if req.include_raw else None}
    }
...
def storyboardn_round1_stream(req: Round1Req):
    ...
    def event_stream():
        ...
        gen, used_hint, fails = generate_pictures_streaming_policy(
            story=req.story, style=req.style,
            min_shots=req.min_shots, max_shots=req.max_shots,
        ...
        tail = {
            "used_model_hint": used_hint, "failures": fails, "shots": len(pics),
            "downloads": {"pictures_url": pics_url, "round1_raw_url": raw_url if req.include_raw else None}
        }
...
def storyboardn_round2_batched(req: Round2BatchedReq):
    kf_json, merged_raw, meta = generate_keyframe_prompts_batched(
        pictures_json=req.pictures, characters=req.characters, scenes=req.scenes,
        batch_size=req.batch_size, max_output_tokens=req.max_output_tokens,
        temperature=req.temperature, continue_segments=req.continue_segments,
    ...
    if not isinstance(kf_json, list) or len(kf_json) == 0:
        raise HTTPException(500, detail={"error": "round2_empty", "failures": meta.get("failures", [])})
    ...
    resp = {
        ...
        "downloads": {"keyframes_url": kf_url, "round2_raw_url": raw_url if req.include_raw else None}
    }
    # 3) 如果缺失镜头仍存在，附带提醒
...
def storyboardn_full(req: FullPipelineReq):
    ...
    if not isinstance(pics_json, list) or len(pics_json) == 0:
        raise HTTPException(500, detail={"error": "round1_empty", "failures": meta1.get("failures", [])})

    # Round2 策略 batched
    kf_json, kf_raw, meta2 = generate_keyframe_prompts_batched(
        pictures_json=pics_json, characters=req.characters, scenes=req.scenes,
        batch_size=req.batch_size, max_output_tokens=req.round2_max_output_tokens,
        temperature=req.round2_temperature, continue_segments=req.round2_continue_segments,
        max_missing_retry_rounds=req.max_missing_retry_rounds,
        parallel_workers=req.parallel_workers or 4,
    )
    if not isinstance(kf_json, list) or len(kf_json) == 0:
        raise HTTPException(500, detail={"error": "round2_empty", "failures": meta2.get("failures", [])})
    ...
    package = {
        ...
        "round1": {"used_model": meta1.get("used_model",""), "failures": meta1.get("failures",[]), "shots": len(pics_json)},
        "round2": {"used_model": meta2.get("used_model",""), "failures": meta2.get("failures",[]), "frames": len(kf_json)},
        "downloads": {"pictures_url": pics_url, "keyframes_url": kf_url, "round1_raw_url": raw1_url, "round2_raw_url": raw2_url}
    }
    # 缺失镜头提醒
    ...
    return {
        "round1": {"used_model": meta1.get("used_model", ""), "failures": meta1.get("failures", []), "shots": len(pics_json)},
        "round2": {
            "used_model": meta2.get("used_model", ""), "failures": meta2.get("failures", []), "frames": len(kf_json),
